...
class WeightedSampler(ImageWindowDatasetCSV):
    """
    ...
    def layer_op(self, idx=None):
        ...
        """
        print('Sampling from Reader with self.mode={}'.format(self.mode))
        data = None
        while data is None:
            image_id, data, _ = self.reader(idx=self.get_idx(self.mode), shuffle=True)
