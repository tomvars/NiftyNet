...
class ApplicationDriver(object):
    ...
    def run_application(self):
        ...
        tf.logging.info('About to build tf.Session...')
        tf.logging.info('config being passed to tf.Session %s' % str(config))
        tf.logging.info('%s' % str(dir(self)))
        for attr, value in self.__dict__.items():
            tf.logging.info('%s %s' % (attr, value))
    ...
    def _create_graph(self, graph=tf.Graph()):
        ...
        tf.logging.info('Building Tensoflow graph...')
        main_device = self._device_string(0, is_worker=False)
        tf.logging.info('main device %s' % main_device)
    ...
    def run_vars(self, sess, message):
        ...
        # with open('timeline_01_%s.json' % message.current_iter, 'w') as f:
    ...
    def _device_string(self, device_id=0, is_worker=True):
        ...
        tf.logging.info('devices %s' % str([x.device_type for x in devices]))
