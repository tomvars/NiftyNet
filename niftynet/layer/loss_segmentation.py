...
def dice(prediction, ground_truth, weight_map=None):
    ...
    if weight_map is not None:
        ...
        tf.logging.info('Dice loss error')
        tf.logging.info('Shape of one_hot op: {}'.format(one_hot.dense_shape))
        tf.logging.info('Shape of prediction op: {}'.format(prediction.shape))
        mul_tensors = one_hot * prediction
        tf.logging.info('Shape of one_hot * prediction: {}'.format(mul_tensors.dense_shape))
        dice_numerator = 2.0 * tf.sparse_reduce_sum(
